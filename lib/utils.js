...
function issueMatch (issueBody, issueConfigs = [], comments) {
  ...
  if (!issueLines || issueLines.length <= 1) {
    return {
      match: false,
      closeComments: [comments.closeComments, comments.titleComment, comments.subtitleComment, comments.contentComment]
    };
  }
  const matches = [match(issueLines, issueConfigs[0], comments), match(issueLines, issueConfigs[1], comments)];
  const templateMatch = matches.find(match => match.currentTemplate);
  if (templateMatch) {
    return {
      match: templateMatch.match,
      closeComments: templateMatch.closeComments
    };
  }
